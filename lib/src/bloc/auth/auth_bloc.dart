...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onAppStarted(AppStarted event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      emit(AuthAuthenticated(user: profile, token: t));
    } catch (e) {
  ...
  Future<void> _onLoginRequested(LoginRequested event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      final dynamic userRaw = res['user'];
      if (userRaw is Map) {
      ...
      emit(AuthAuthenticated(user: userMap, token: token));
    } catch (e) {
  ...
  Future<void> _onFetchProfileRequested(FetchProfileRequested event, Emitter<AuthState> emit) async {
    ...
    try {
      final profile = await repository.getProfile();
      emit(AuthAuthenticated(user: profile, token: repository.token));
    } catch (e) {
      emit(AuthFailure(message: e.toString()));
    }
  }
  ...
  Future<void> _onUpdateProfileRequested(UpdateProfileRequested event, Emitter<AuthState> emit) async {
    ...
    try {
      ...
      if (patchedUser == null) {
        ...
        emit(AuthAuthenticated(user: refreshed, token: repository.token));
      } else {
        emit(AuthAuthenticated(user: patchedUser, token: repository.token));
      }
    } catch (e, st) {
